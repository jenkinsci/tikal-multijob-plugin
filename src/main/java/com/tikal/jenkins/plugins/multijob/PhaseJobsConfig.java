...
public class PhaseJobsConfig implements Describable<PhaseJobsConfig> {
    ...
    public static class DescriptorImpl extends Descriptor<PhaseJobsConfig> {
        ...
        public AutoCompletionCandidates doAutoCompleteJobName(@QueryParameter String value) {
            ...
            for (String localJobName : Jenkins.get().getJobNames()) {
                if (localJobName.toLowerCase().startsWith(value.toLowerCase())) c.add(localJobName);
            }
            return c;
        }

        private void savePhaseJobConfigParameters(String localJobName) {
            AbstractProject project = ((AbstractProject) Jenkins.get().getItemByFullName(localJobName));
            ...
            for (ParameterDefinition pdef : parameterDefinitions) {
                ...
                if (pdef instanceof StringParameterDefinition) {
                    ...
                    paramValue = (String)
                            stringParameterDefinition.getDefaultParameterValue().getValue();
        ...
        private static AbstractProject getCurrentJob() {
            ...
            return (AbstractProject) Jenkins.get().getItemByFullName(jobName);
        }
    ...
    public List<ParameterValue> getJobParameters(AbstractBuild<?, ?> build, TaskListener listener) {
        ...
        List<ParameterValue> values =
                new ArrayList<ParameterValue>(action.getParameters().size());
    ...
    private static MultiJobParametersAction mergeParameters(
            MultiJobParametersAction base, MultiJobParametersAction overlay) {
        LinkedHashMap<String, ParameterValue> params = new LinkedHashMap<String, ParameterValue>();
        for (ParameterValue param : base.getParameters()) if (param != null) params.put(param.getName(), param);
    ...
    public List<Action> getActions(AbstractBuild build, TaskListener listener, Job project, boolean isCurrentInclude)
        ...
        if (parameters != null) {
            ...
            params =
                    new MultiJobParametersAction(paramsValuesList.toArray(new ParameterValue[paramsValuesList.size()]));
        }
            ...
            for (AbstractBuildParameters config : configs) {
                ...
                try {
                    ...
                    if (a instanceof ParametersAction) {
                        MultiJobParametersAction mjpa =
                                new MultiJobParametersAction(((ParametersAction) a).getParameters());
                        if (params == null) {
                            params = mjpa;
                        } else {
                            params = mergeParameters(params, mjpa);
    ...
    public Object readResolve() {
        if (hasProperties()) {
            ...
            if (configs == null) configs = new ArrayList<AbstractBuildParameters>();
        ...
        if (configs != null && configs.size() > 0) {
            ...
            while (parametersIterator.hasNext()) {
                ...
                if (param instanceof com.tikal.jenkins.plugins.multijob.PredefinedBuildParameters) {
                    com.tikal.jenkins.plugins.multijob.PredefinedBuildParameters previosStringParam =
                            (com.tikal.jenkins.plugins.multijob.PredefinedBuildParameters) param;
                    ...
                    com.tikal.jenkins.plugins.multijob.FileBuildParameters previosFileParam =
                            (com.tikal.jenkins.plugins.multijob.FileBuildParameters) param;
                    parametersIterator.remove();
