                ...
                if (builders != null) {
                    for (Builder builder : builders) {
                        if (builder instanceof MultiJobBuilder) {
                            ...
                            changed |= multiJobBuilder.onJobRenamed(oldName, newName);
                            if (changed)
                                try {
                                    project.save();
                                } catch (IOException e) {
                                    Logger.getLogger(MultiJobListener.class.getName())
                                            .log(
                                                    Level.WARNING,
                ...
                if (builders != null) {
                    for (Builder builder : builders) {
                        if (builder instanceof MultiJobBuilder) {
                            ...
                            if (changed)
                                try {
                                    int phaseJobsCounter =
                                            multiJobBuilder.getPhaseJobs().size();
                                    ...
                                    Logger.getLogger(MultiJobListener.class.getName())
                                            .log(
                                                    Level.WARNING,
                                                    "Failed to persist project setting during remove of " + oldName,
                                                    e);
                                }
                        }
                    }
